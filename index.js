...
const plugin_manager = require('./Source_JS/plugin_manager.js');
...
function boot() {
	...
	window.loadURL(url.format({
		pathname: path.join(__dirname, './Source_HTML/index.html'),
		protocol: 'file:',
		slashes: true
	}));
...
ipcMain.on("change_place_details", function(event, args) {
	...
	place_window.loadURL(url.format({
		pathname: path.join(__dirname, './Source_HTML/change_place_details.html'),
		protocol: 'file:',
		slashes: true
	}));
});
...
ipcMain.on("change_component_details", function(event, args) {
	...
	component_window.loadURL(url.format({
		pathname: path.join(__dirname, './Source_HTML/change_component_details.html'),
		protocol: 'file:',
		slashes: true
	}));
});
...
ipcMain.on("change_transition_details", function(event, args) {
	...
	transition_window.loadURL(url.format({
		pathname: path.join(__dirname, './Source_HTML/change_transition_details.html'),
		protocol: 'file:',
		slashes: true
	}));
});
